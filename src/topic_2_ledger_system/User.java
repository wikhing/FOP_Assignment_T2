...
public class User {
    ...
    List<String[]> fullTransactionList = new ArrayList<>();
    ...
    User(int user_id){
        for(String[] user : fullUserList){
            if(user_id == Integer.parseInt(user[0])){
                ...
                this.fullTransactionList = file.get_transactions_csv(user_id);
            }
        }
    }
    
    static List<String[]> fullUserList = new ArrayList<>();
    ...
    public static void load(){
        ...
        for(int i = 1; i < user_csv.size(); i++){
            ...
            fullUserList.add(temp);
        }
        
        for(int i = 0; i < fullUserList.size(); i++){
            String user_id = fullUserList.get(i)[0];
            
            for(int j = 1; j < saving_csv.size(); j++){
                if(user_id.equals(saving_csv.get(j)[1])){
                    fullUserList.get(i)[4] = saving_csv.get(j)[0];
                    fullUserList.get(i)[5] = saving_csv.get(j)[2];
                    fullUserList.get(i)[6] = saving_csv.get(j)[3];
                    fullUserList.get(i)[7] = saving_csv.get(j)[4];
                    break;
                }
            }
            ...
            for(int j = 1; j < loan_csv.size(); j++){
                if(user_id.equals(loan_csv.get(j)[1])){
                    fullUserList.get(i)[8] = loan_csv.get(j)[0];
                    fullUserList.get(i)[9] = loan_csv.get(j)[2];
                    fullUserList.get(i)[10] = loan_csv.get(j)[3];
                    fullUserList.get(i)[11] = loan_csv.get(j)[4];
                    fullUserList.get(i)[12] = loan_csv.get(j)[5];
                    fullUserList.get(i)[13] = loan_csv.get(j)[6];
                    fullUserList.get(i)[14] = loan_csv.get(j)[7];
                    break;
                }
            }
            ...
            for(int j = 1; j < balance_csv.size(); j++){
                if(user_id.equals(balance_csv.get(j)[0])){
                    fullUserList.get(i)[15] = balance_csv.get(j)[1];
                    break;
                }
            }
        }
